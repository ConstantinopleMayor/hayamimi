...
- **背景可调**：文字后的黑色圆角卡片（文字本身始终不透明），卡片**覆盖整个顶部条**——按钮行垫在卡片之上，文字始于按钮下方 8px；空态（无字幕）时卡片保持一行文字的高度，不会缩到文字上方。透明度用**顶部滑块**调节（0–60%，步进 5；设置菜单同步提供 13 档）。命令行可用 `--bg 30`、`--mode tr`、`--bold` 设定默认值。**两个滑块（透明度 + 宽度）并排放在顶部条、左锚定**——拖动时不会漂移。
- **字幕粗体**：设置菜单的「字幕粗体」勾选项（或 `--bold` 参数）可让原文与译文两行同开粗体。

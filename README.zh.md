...
- **背景可调**：文字后的黑色圆角卡片（文字本身始终不透明），卡片**覆盖整个顶部条**——按钮行垫在卡片之上，文字始于按钮下方 8px；空态（无字幕）时卡片保持一行文字的高度，不会缩到文字上方。透明度用**顶部居中滑块**调节（0–60%，步进 5；设置菜单同步提供 13 档）。命令行可用 `--bg 30`、`--mode tr`、`--bold` 设定默认值。
...
- 点击穿透模式下，左上角按钮带与右上角窗口按钮**仍保持可点击**（其余区域全部穿透）。

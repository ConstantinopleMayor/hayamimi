...
- **累积字幕流**：确认的 `final` 会在屏幕上**累积显示**（多条可同屏、一起换行），每条确认段——原文及其翻译——各自保持 **8 秒**后**立即移除（无淡出）**。说话过程中 `partial`（进行中草稿）以斜体排在流末尾同行动态更新。
- 字幕自上而下流动，窗口**高度自动贴合内容**（仅留 4px 底部边距，窗口真正贴住卡片——下方不再有吞滚轮的空白条），**宽度可用顶部滑块调节**。
- **显示模式切换**：按钮在**双语**（原文 + 译文两行，默认）与**仅译文**（隐藏原文流）之间切换；设置菜单中同样可选。
- **背景可调**：文字后的黑色圆角卡片（文字本身始终不透明），卡片**覆盖整个顶部条**——按钮行垫在卡片之上，文字始于按钮下方 8px；空态（无字幕）时卡片保持一行文字的高度，不会缩到文字上方。透明度用**顶部滑块**调节（0–60%，**步进 1** 可微调；设置菜单保留常用档位），**启动默认 1%**（几乎察觉不到的淡卡片）。命令行可用 `--bg <0-60>`、`--mode tr`、`--bold` 设定默认值。**两个滑块（透明度 + 宽度）并排放在顶部条、左锚定**——拖动时不会漂移。
- **文字样式可调**：设置菜单四组单选（启动默认可用 `--text-*` 参数设定），同时作用于原文与译文两行。**启动默认：粗体 + 标准阴影 + 重墨**（细描边与重墨互斥，默认关闭）：
  - **阴影**：无 / 标准（默认，`--text-shadow std`，1px 光晕 + 0.5px 浓核 + 1px 1px 方向投影）/ 增强（`heavy`）
  - **描边**：无（默认）/ 细描边（`--text-stroke thin`，发丝级 0.15pt 外描边——采用 B 站弹幕 DOM 式 8 方向偏移阴影模拟，描边恒在字形外侧，不会像 `-webkit-text-stroke` 那样盖住笔画内部；与重墨互斥）
  - **重墨**：无 / 重墨（默认，`--text-ink on`，B 站弹幕风格 8 方向 0.3pt 轮圈 + 0.2pt 圆角柔化，同为外侧偏移阴影技法，与描边互斥）
  - **文字透明度**：100（默认）/90/80/70/60%（`--text-opacity <pct>`，背景卡片的透明度是另一项独立设置）
- **字幕粗体**：**默认开启**；「字幕粗体」勾选项可在设置菜单关闭（`--bold` 因默认即开而成为空操作）。
- **窗口宽度**：顶部滑块可调窗口宽度 500–1200px（步进 10），文字随宽度重排、高度继续自动贴合。（此透明无框窗口没有原生缩放手柄，故用滑块驱动。）
- 左上角固定按钮：
  - **🔒 / 🔓** —— 切换"交互拖拽模式 / 点击穿透模式"。
  - **⚙** —— 设置菜单（字号、字体、翻译语言、显示模式、背景透明度、点击穿透、退出）。

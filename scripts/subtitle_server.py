...
class SubtitleServer:
    ...
    def final(self, text: str, lang: str = "", speaker: str = "",
              latency_ms: float | None = None, tier: str = "", seq: int | None = None):
        """Publish a finalized subtitle line. `seq` (monotonic utterance id)
        lets front-ends attach the async translation events to the correct line."""
        self.publish({"type": "final", "text": text, "lang": lang,
                      "speaker": speaker, "latency_ms": latency_ms,
                      "tier": tier, "seq": seq})

...
The bug these guard (upstream #55, ported into this fork): `pytest tests`
grew to ~19 GB RSS because neither RoutedASR nor Refiner could ever be
collected -- each had started daemon threads that never return
(Refiner._worker_loop and TranslationWorker's final/partial loops'
...
holds several GB of sherpa-onnx recognizers.
...
real ja pipeline is skipped without models/ + golden clips -- sherpa-onnx's
C++ layer calls exit() rather than raising when handed a missing model
path, so there is no "try it and see" here.
"""
...
# ---- TranslationWorker -----------------------------------------------------
# (this fork's worker runs TWO threads: the finals loop and the polled
# partial-draft loop -- both must end, and both submits must degrade to
# no-ops afterwards.)

def test_translation_worker_close_ends_its_threads(no_thread_leak):
    worker = TranslationWorker({})
    assert worker._final_thread.is_alive()
    assert worker._partial_thread.is_alive()
    worker.close()
    worker.close()  # idempotent
    assert not worker._final_thread.is_alive()
    assert not worker._partial_thread.is_alive()
...
def test_translation_worker_submit_after_close_is_a_no_op(no_thread_leak):
    ...
    assert not worker.submit_partial("draft text")  # must not raise either
    assert worker._q.empty()
    assert worker._partial_q.empty()


def test_translation_worker_hot_swap_survives_close(no_thread_leak):
    """set_langs() (this fork's hot-toggle path) after close must stay
    harmless: it only swaps a dict under the lock."""
    worker = TranslationWorker({})
    worker.close()
    worker.set_langs({})
...
def stub_engine_builders(monkeypatch):
    ...
    build in __init__, the _BUILDERS, the on-disk presence check, and the
    warm-up decode _preload_rest runs per tier. The punct/ko_spacer
...
def test_run_clip_leaves_no_engine_behind(no_thread_leak):
    """A full per-clip pipeline loop used to mean one live engine (~2.4 GB
    each) per clip for the rest of the session; now the second pass starts
    from where the first one did."""
    import make_ja_golden  # noqa: F401 (only meaningful when golden data exists)
    ...
    assert alive == [], (
        ...
        "holds its recognizers for the life of the process")
